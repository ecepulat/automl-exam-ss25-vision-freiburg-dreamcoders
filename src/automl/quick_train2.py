...
class QuickTrain:
    def __init__(self, dataset_name="flowers", batch_size=25, epochs=6):
        ...
        with open(self.LOG_FILE, "w") as f:
            f.write("\U0001F4DD EfficientNet-B4 Training Log\n\n")
        ...
        with open(self.LOG_FILE, "a") as f:
            f.write(f"\U0001F4CF Total Parameters: {params}\n")
            f.write(f"\U0001F6E0️ Hyperparameters:\n")
    ...
    def _load_metadata(self):
        ...
        if not os.path.exists(metadata_path):
            print("\U0001F4C9 Metadata file not found — running data_analyze.py ...")
            import subprocess
            subprocess.run([
                sys.executable,
    ...
    def _prepare_data(self):
        ...
        base_path = project_root / "data" / self.dataset_name
        df = pd.read_csv(os.path.join(base_path, "train.csv"))
        test_df = pd.read_csv(os.path.join(base_path, "test.csv"))
        label_counts = Counter(test_df['label'])
        with open("testset.txt", "w") as f:
            f.write("\U0001F4CA Test set class distribution:\n")
        ...
        print(f"\U0001F4CA Training samples (balanced): {len(train_dataset)}")
        final_counts = Counter(label for _, label, _ in train_dataset.data)
        print(f"\U0001F9AE Final training class distribution (after balancing): {dict(final_counts)}")
        print(f"\U0001F9EA Test samples: {len(self.test_loader.dataset)}")

    def evaluate_test(self):
        print("\U0001F9EA Evaluating on test set...")
        self.model.eval()
        all_preds = []
        all_labels = []
    ...
    def train(self):
        print("\U0001F680 Starting quick training run...")
        if torch.cuda.is_available():
            gpu_name = torch.cuda.get_device_name()
            total_mem = torch.cuda.get_device_properties(0).total_memory / 1024**3
            with open(self.LOG_FILE, "a") as f:
                f.write(f"\U0001F5A5️ GPU: {gpu_name} | Memory: {total_mem:.2f} GB\n\n")
        ...
        for epoch in range(self.epochs):
            ...
            print(f"\U0001F4E6 Epoch {epoch+1}: Loss={avg_loss:.4f}, Accuracy={acc:.4f}")
            ...
            with open(self.LOG_FILE, "a") as f:
                ...
                f.write(f"\U0001F9EA Validation Accuracy after Epoch {epoch+1}: {val_acc:.4f}\n")
